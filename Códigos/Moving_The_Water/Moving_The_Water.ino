...
void loop(void) {
  ...
  if ( previousValue1 != 0 && formatWaterLevel(waterLevel1) != 0){
    ...
    Serial.print(" - ");
    if ( moveWater2(previousValue1,previousValue2,formatWaterLevel(waterLevel1),formatWaterLevel(waterLevel2)) == 1 ){
...
int moveWater2 ( float pValue1, float pValue2, float value1, float value2 ){
  if  ( value1 > (pValue1 + 0.3) || value2 > (pValue2 + 0.3) ){
    return 1;
  }
  else{
    return 0;
